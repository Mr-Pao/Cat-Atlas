...
labels = [
    ...
    [15, '最近一次目击', lambda x: str(x)],
    [20, '领养时间', lambda x:str(x)],
    [21, '离世时间', lambda x:x],
    [13, '外貌', lambda x:x],
    [18, '更多', lambda x:x],
...
for i in range(rowNum):
    if data_list[i][3] != '':
        ...
        if (data_list[i][9] == '健康' or data_list[i][9] == '口炎'):
            ...
            if data_list[i][6] == 6:
                qita.append(data_list[i][2])

health = lihua + ju + nainiu + sanhua + chunse + qita
suoyou = health
...
# 三花
if not os.path.exists('index/三花'):
    os.makedirs('index/' + '三花')  # 创建每只猫的文件夹
    # 创建js文件
with open('index/三花/三花' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in sanhua:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
...
# 大橘
if not os.path.exists('index/大橘'):
    os.makedirs('index/' + '大橘')  # 创建每只猫的文件夹
    # 创建js文件
with open('index/大橘/大橘' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in ju:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
...
# 其它
if not os.path.exists('index/其它'):
    os.makedirs('index/' + '其它')  # 创建每只猫的文件夹
    # 创建js文件
with open('index/其它/其它' + '.js', 'w', encoding='utf-8') as f:
    f.write('var app = getApp()\n Page({\ndata: { \n catlist: [\n')
    for name in qita:
        f.write('{ name:"'+name+'"},')
    with open('js2.txt', 'r', encoding='utf-8') as f2:
